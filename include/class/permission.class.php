...
*/
	...
	public static function userPerm($uid){
		global  $webdb;
	...
	public static function errMsg($str='對不起您沒有該操作的權限'){
		jsCtrl::Alert($str);
		exit;
	}
	public static function getSection($table_name){
	...
	public static function get_group_id($admin_id){
		if(empty($admin_id)) return false;
		global $webdb;
		return $webdb->getValue("select * from _sys_admin where id=".$admin_id,"gpid");
	}
	public static function check($table,$action,$admin_id=null){
		global $webdb,$admin_folder;
		if(!$admin_folder) return true;
