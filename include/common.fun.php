...
function easyhtmlEdit($name, $value = "", $height = 60, $width = null, $toolbar = array(), $skin = 'moonocolor', $Lang = 'zh')
{
    // global $rooturl;
    $CKEditor           = new CKEditor();
    $CKEditor->basePath = ROOT_URL . 'ckeditor/';
    $config             = array();
    if ($height) {
    ...
    $config['toolbar'] = array(
        // array('Superscript'),
        array('Source', 'Preview', '-', 'Undo', 'Redo', 'Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', 'SelectAll'),
        // array('Table','HorizontalRule','SpecialChar'),
        // array('Outdent','Indent','Blockquote'),
        array('Bold', 'Italic', 'Underline', 'Strike'),
        array('JustifyLeft', 'JustifyCenter', 'JustifyRight', 'JustifyBlock'),
        // array('Superscript','Subscript','-','NumberedList','BulletedList'),
...
function htmlEdit($name, $value = "", $height = 500, $width = null, $toolbar = array(), $skin = 'moonocolor', $Lang = 'zh')
{
    ...
    $config['filebrowserBrowseUrl']      = ROOT_URL . 'ckeditor/ckfinder/ckfinder.html';
    $config['filebrowserImageBrowseUrl'] = ROOT_URL . 'ckeditor/ckfinder/ckfinder.html?type=Images';
    // $config['filebrowserFlashBrowseUrl'] = ROOT_URL.'ckeditor/ckfinder/ckfinder.html?type=Flash';
    $config['filebrowserUploadUrl']      = ROOT_URL . 'ckeditor/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Files';
    $config['filebrowserImageUploadUrl'] = ROOT_URL . 'ckeditor/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Images';
    // $config['filebrowserFlashUploadUrl'] = ROOT_URL.'ckeditor/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Flash';
    $config['toolbar'] = array(
        array('Source', 'Preview'),
        array('Undo', 'Redo', '-', 'Cut', 'Copy', 'Paste', 'PasteText', 'PasteFromWord', 'SelectAll'),
        array('JustifyLeft', 'JustifyCenter', 'JustifyRight', 'JustifyBlock'),
        array('Link', 'Unlinkk', 'Anchor', '-', 'Image', 'oembed'),
...
function cutstrs($str, $length)
{
    return substrs(trim(str_replace('　', '', str_replace('&nbsp;', '', strip_tags($str)))), $length);
}
/**
 * 獲取IP
 */
...
function adshow($name, $page = null)
{
    ...
    if (substr($ad_data['img'], 0, 7) != 'http://') {
        $ad_data['img'] = ROOT_URL . $ad_data['img'];
    }
    $htmlstr = '';
...
function getOne($table, $where = "", $field = "*")
{
    ...
    return $webdb->getValue("select " . $field . " from " . $table . " where 1" . $where, ($field != "*" ? $field : ""));
}
...
function getSmallPic($pic)
{
    ...
    // print_r($path_parts);
    $small_pic = $path_parts['dirname'] . '/' . $path_parts['filename'] . '_small.' . $path_parts['extension'];
