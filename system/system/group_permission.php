							...
							<div class="card-body">
								...
								<div class="col-md-4">
										<div class="form-group row">
												<div class="col-sm-12">
<select name="group_id" class="form-control" onchange="window.location.href='<?php echo urlkill('group_id')?>&group_id='+this.value">
<?php foreach ($group as $gp) {?>
<option <?php if ($_GET['group_id']==$gp['id']) {
    echo 'selected';
}?> value="<?php echo $gp['id']?>"><?php echo $gp['name']?></option>
<?php }?>
</select>
												</div>
										</div>
...
function power_selectAll(select_name,input_name){
	...
	// $("input[id='"+select_name+"']").click( function (){
		// $("input[id*='"+input_name+"']").prop("checked",this.checked); 
	// }); 
